...
bool YOLO_SEG::init()
{
	...
	if (!m_usePlugin)
	{
		m_total_objects = m_detect_dims.d[2]; // 预测目标数量 80*80+40*40+20*20
		m_classes_nums = m_detect_dims.d[1] - 36; // 类别数量
...
void YOLO_SEG::postprocess(std::vector<Detection>& res, std::vector<cv::Mat>& masks)
{
	...
	cuda_decodeSeg((float*)m_buffers[1], m_output_objects_device, 1, m_total_objects, m_classes_nums, m_cfg.max_det, m_cfg.conf_threshold, m_stream);
...
{
	CUDA_CHECK(cudaMemsetAsync(m_output_objects_device, 0, sizeof(float) * (m_cfg.max_det * m_output_objects_width + 1), m_stream));
	cuda_decodeSeg((float*)m_buffers[1], m_output_objects_device, m_cfg.batch_size, m_total_objects, m_classes_nums, m_cfg.max_det, m_cfg.conf_threshold, m_stream);
